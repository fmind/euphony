...
(defonce TRUTHS
  [{:ground-truth/resource "f63256cf4eef0a60fe56989b1474dd9b0b2bb580ce9fd262b18592bf0506f911"
   ...
   {:ground-truth/resource "a0196e43aaf90bef85b9661ec23037c79d246d94e1100192295079529d529d97",
    :ground-truth/name "singleton:a0196e43", :ground-truth/plat "android"}
   {}])
...
(t/deftest can-query-mem>truth->attribute
  (t/is (= (mem>truth->attribute :ground-truth/name TRUTHS)
  ...
  (t/is (= (mem>truth->attribute :ground-truth/type TRUTHS)
  ...
  (t/is (= (mem>truth->attribute :ground-truth/plat TRUTHS)
...
(t/deftest can-query-db>word->field
  (let [index (db>word->field (d/db ts/*conn-after-import*))]
...
(t/deftest can-query-db>label->antivirus
  (t/are [label, output] (= (db>label->antivirus label (d/db ts/*conn-after-import*)) output)
    "ads", []
    "ads.adrd", ["av1"]
    "android.koguo.1", ["av3" "av4"]))

(t/deftest can-query-db>label->attribute
  (t/is (= (db>label->attribute :antivirus.label/name-part (d/db ts/*conn-after-parse*))
           {"ads.adrd" "adrd"
  ...
  (t/is (= (db>label->attribute :antivirus.label/type-part (d/db ts/*conn-after-parse*))
           {"ads.adrd" "ads"
  ...
  (t/is (= (db>label->attribute :antivirus.label/plat-part (d/db ts/*conn-after-parse*))
           {"andr.adrd" "andr"
...
(t/deftest can-query-db>result->attribute
  (t/is (= (db>result->attribute :antivirus.result/name-cluster (d/db ts/*conn-after-cluster*))
           {"av1++ads.adrd" "adrd"
            ...
            "av5++generic" "generic"}))
  (t/is (= (db>result->attribute :antivirus.result/type-cluster (d/db ts/*conn-after-cluster*))
           {"av1++ads.adrd" "adware"
  ...
  (t/is (= (db>result->attribute :antivirus.result/plat-cluster (d/db ts/*conn-after-cluster*))
           {"av3++android.adrd.1" "android"
            "av3++android.koguo.1" "android"
            "av3++android.pjapps.1" "android"})))

(t/deftest can-query-db>truth->attribute
  (t/is (= (db>truth->attribute :ground-truth/name (d/db ts/*conn-after-import*))
           {"f1" "dogwin"
            "f2" "dogwin"
            "f3" "dogwin"
            "f4" "adrd"
            "f5" "adrd"}))
  (t/is (= (db>truth->attribute :ground-truth/type (d/db ts/*conn-after-import*))
           {"f1" "trojan"
            "f2" "trojan"
            "f3" "trojan"
            "f4" "adware"
            "f5" "adware"}))
  (t/is (= (db>truth->attribute :ground-truth/plat (d/db ts/*conn-after-import*))
           {"f1" "android"
            "f2" "android"
            "f3" "android"
            "f4" "android"
...
(t/deftest can-query-db>result->av_label
  (t/is (= (db>result->av_label :antivirus.label/name-part (d/db ts/*conn-after-parse*))
  ...
  (t/is (= (db>result->av_label :antivirus.label/type-part (d/db ts/*conn-after-parse*))
  ...
  (t/is (= (db>result->av_label :antivirus.label/plat-part (d/db ts/*conn-after-parse*))
...
(t/deftest can-query-db>report->label-results
  (t/is (= (db>report->vendor-results :antivirus.label/name-part (d/db ts/*conn-after-parse*))
...
(t/deftest can-query-db>report->cluster-results
  (t/is (= (db>report->cluster-results :antivirus.result/name-cluster (d/db ts/*conn-after-cluster*))
...
(t/deftest can-query-db>words-with-this-field
  (t/are [field, subset] (th/subset? subset (db>words-with-this-field field (d/db ts/*conn-initial*)))
    :Z, #{}
    :I, #{"dll" "pak" "gen"}
...
(t/deftest can-query-db>labels-with-unknown-fields-pattern
  (t/is (th/set= (db>labels-with-unknown-fields-pattern (d/db ts/*conn-after-parse*)) #{"dogwin.b" "dogwin"})))

(t/deftest can-query-db>fields-patterns-related-to-label
  (t/are [label, output] (= (db>fields-patterns-related-to-label (d/db ts/*conn-after-parse*) label) output)
